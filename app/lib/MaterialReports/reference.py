...
def metLib(inputDF):
    ...
    #df['На металл']                 = df.apply(lambda x: src.loc[ src['Код товара'] == x['Код товара'], 'На металл' ].unique()[0], axis = 1)
    #df['Алюминий, кг']              = df.apply(lambda x: src.loc[ src['Код товара'] == x['Код товара'], 'Алюминий, кг' ].unique()[0]*x['Количество (из акта)'], axis = 1)
    #df['Нержавейка, кг']            = df.apply(lambda x: src.loc[ src['Код товара'] == x['Код товара'], 'Нержавейка, кг' ].unique()[0]*x['Количество (из акта)'], axis = 1)
    #df['Черный металл, кг']         = df.apply(lambda x: src.loc[ src['Код товара'] == x['Код товара'], 'Черный металл, кг' ].unique()[0]*x['Количество (из акта)'], axis = 1)
    #df['Драг. металл, кг']          = df.apply(lambda x: src.loc[ src['Код товара'] == x['Код товара'], 'Драг. металл, кг' ].unique()[0]*x['Количество (из акта)'], axis = 1)
