...
department_Filters = {
    'rmpd': [{"field": 'isRMPD_planner', "operator": "==", "value": "'yes'"},
             "&",
             {"field": 'Reserved By', "operator": "!=",
              "value": "'Mirjahon Toirov CofE'"}
             ],
    'cofe': [{"field": 'Reserved By', "operator": "==", "value": "'Bobur Aralov'"},
             "|",
             {"field": 'Reserved By', "operator": "==",
              "value": "'Mirjahon Toirov CofE'"}
             ]
...
is_014 = ['06935', '05841', '07139', '05230', '04189', '03494', '02799', '05749', '01738', '01308', '01309', '01310', '01311', '01312', '01329', '01330', '01331', '01333', '01351', '01354', '01372', '01374', '01376',
          ...
          '06900', '07497', '07883', '07882', '09300', '07331', '05831', '09767', '01875', '04183', '09359', '02811', '07626', '07627', '07628', '07629', '07616', '07617', '10920', '05157',
          '08008', '03014', '06326', '09297', '00704', '05824', '09605', '09606', '12414', '09608', '01517', '02838', '11063', '02794', '10518', '12500', '27290', '24631',
          ]

is_wOff = ['09327', '09326', '07483', '12478', '09859',
           '07536', '05328', '06949', '09607', '16801',]
...
def metLib(inputDF):
    ...
    ### Subproducts
    sub = {
        #'07379':{'На уничтожение':'No', 'В повторное использование':'No', 'На металл':'Yes', 'Алюминий, кг':0, 'Нержавейка, кг':0, 'Черный металл, кг':0, 'Драг. металл, кг':0 },
        '07379':{'На уничтожение':'No', 'В повторное использование':'No', 'На металл':'Yes', 'Алюминий, кг':0, 'Нержавейка, кг':0, 'Черный металл, кг':0, 'Драг. металл, кг':0 },
    }
    for item, value in sub.items():
        df.loc[ df['Код товара']==item, 'На уничтожение']            = value['На уничтожение']
        df.loc[ df['Код товара']==item, 'В повторное использование'] = value['В повторное использование']
        df.loc[ df['Код товара']==item, 'На металл']                 = value['На металл']

        df['Алюминий, кг']      = df.apply(lambda x: x['Количество (из акта)'] * value['Алюминий, кг']      if x['Код товара']==item else x['Алюминий, кг'], axis=1)
        df['Нержавейка, кг']    = df.apply(lambda x: x['Количество (из акта)'] * value['Нержавейка, кг']    if x['Код товара']==item else x['Нержавейка, кг'], axis=1)
        df['Черный металл, кг'] = df.apply(lambda x: x['Количество (из акта)'] * value['Черный металл, кг'] if x['Код товара']==item else x['Черный металл, кг'], axis=1)
        df['Драг. металл, кг']  = df.apply(lambda x: x['Количество (из акта)'] * value['Драг. металл, кг']  if x['Код товара']==item else x['Драг. металл, кг'], axis=1)
